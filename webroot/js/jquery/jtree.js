...
Amendment by Stephen Neate at line 188
 */
(function($) {
		...
		var putPlacement = function(e){
			...
			//inserting after
				...
				*/
				...
				// ######  alteration to grab previous ID off the li used for easy serialization
				else if (e.pageX > off.left + opts.childOff) {
					$(".jTreePlacement").remove();
					if ($(this).find("ul").length == 0){
						var newVal = $(this).prev("li").next("li").attr("id").split("item_"); 	// new line
						$(this).append('<ul id="parent-'+newVal[1]+'">'+str+'</ul>'); 					// amended code
					}else{
						$(this).find("ul").prepend(str);
						}
				}
